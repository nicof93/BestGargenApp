...
public class MathUtils {
	...
	public void rotate(PointF p1, PointF p2, float angle) {
		...
		float ox = p2.x;
		float oy = p2.y;
		p1.x = Float.parseFloat(String.valueOf((Math.cos(angle) * (px-ox) - Math.sin(angle) * (py-oy) + ox)));
		p1.y = Float.parseFloat(String.valueOf((Math.sin(angle) * (px-ox) + Math.cos(angle) * (py-oy) + oy)));
	}
